...
local CONFIG_FILE = "AWhub_Settings_V16.json"
...
local function sendAppleWareWebhook(title, description, fields, color)
    ...
    local embed = {
        ...
        fields = fields or {},
        footer = {
            text = "AppleWare • https://applewareh.vercel.app"
        },
...
local function sendStatusWebhook()
    ...
    local rate = math.floor(totalCoinsEarned / math.max(elapsed / 3600, 0.01))

    sendAppleWareWebhook(
        "AWhub Status Update",
        "Periodic progress update for **" .. LP.Name .. "**.",
        {
            {name = "Player", value = LP.Name .. "\n`" .. LP.UserId .. "`", inline = true},
            {name = "Coins", value = "Current Bag: **" .. currentCoinCount .. "/" .. maxCoinCount .. "**\nSession: **" .. totalCoinsEarned .. "**", inline = true},
            {name = "Statistics", value = "Time: **" .. minutes .. ":" .. string.format("%02d", seconds) .. "**\nRate: **" .. rate .. "/hr**", inline = true},
            {name = "Server", value = "Place: `" .. game.PlaceId .. "`\nJob ID: `" .. game.JobId .. "`", inline = false}
        },
        0x57F287
    )
end

-- Timer-based background task loop for webhooks
task.spawn(function()
    while true do
...
local function buildUI()
    ...
    timerBtn.Size = UDim2.new(1, 0, 0, 32)
    timerBtn.Position = UDim2.new(0, 0, 0, 38)
    ...
    timerBtn.TextSize = 11.5
    timerBtn.TextXAlignment = Enum.TextXAlignment.Left
    timerBtn.Parent = pages["Webhook"]
    Instance.new("UICorner", timerBtn).CornerRadius = UDim.new(0, 6)
    ...
    whBox.Position = UDim2.new(0, 0, 0, 76)
    ...
    idBox.Position = UDim2.new(0, 0, 0, 114)
    ...
    fb.MouseButton1Click:Connect(function()
        ...
        if isOpen then
            f.Visible = true
            TweenService:Create(f, TweenInfo.new(0.3), {Size = UDim2.new(0, 420, 0, 380)}):Play()
            fb.Text = "close"
        else
            TweenService:Create(f, TweenInfo.new(0.25), {Size = UDim2.new(0, 420, 0, 0)}):Play()
